<?php
    ...
    $query_student_by_ID = "SELECT * FROM sinh_vien_tbl WHERE id_sv=$id";

    $student_result = $con->query($query_student_by_ID);

    if (mysqli_num_rows($student_result) == 1) {
        $student = mysqli_fetch_assoc($student_result);
        $name = $student['ten'];
        $birth = $student['ngay_sinh'];
        $gender = $student['gioi_tinh'];
        $phone = $student['so_dien_thoai'];
        $email = $student['email'];
        $address = $student['dia_chi'];
        $class = $student['id_lop'];
        $join = $student['ngay_vao_hoc'];
        $avatar = $student['anh'];
    } 
    ...
    if (isset($_POST['submit'])) {
        ...
        if (!empty($name) && !empty($email)  && !empty($avatar) && !empty($birth) && !empty($gender) && !empty($address) && !empty($phone) && !empty($class) && !empty($join) && checkErr($nameErr, $emailErr, $genderErr, $addressErr, $phoneErr)) {
            $query = "UPDATE sinh_vien_tbl
            SET ten='$name',ngay_sinh='$birth',gioi_tinh='$gender',so_dien_thoai='$phone',email='$email',dia_chi='$address',id_lop='$class',ngay_vao_hoc='$join',anh='$avatar',ngay_cap_nhat='$join' 
            WHERE id_sv=$id";
                    ...
                    <div class="row flex-nowrap">
                        ...
                        <div class="col py-3">
                            ...
                            <div class="card card-registration">
                                ...
                                <h3 class="mb-4 pb-2 pb-md-0 mb-md-5">Cập nhật thông tin sinh viên</h3>
                                    <form method="post">
                                        ...
                                        <div class="row">
                                            <div class="col-md-4 mb-4 pb-2">
                                                ...
                                                <select class="form-select form-control-lg" name="select_class">
                                                    <option value="1" disabled>Lớp</option>
                                                    <?php
